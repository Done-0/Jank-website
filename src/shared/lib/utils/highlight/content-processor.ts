...
const quotePatterns: QuotePattern[] = [
    { pattern: /<p>["'""](.+?)["'""](?:\s*.*?)?<\/p>/g, replacement: '<blockquote><p>$1</p></blockquote>' },
    {
        pattern: /(<p>["'""].*?["'""](?:\s*.*?)?<\/p>)\s*(<p>.*?<\/p>)/g,
        handler: (m: string, q: string, n: string) => n.includes(' said') ? `<blockquote>${q}${n}</blockquote>` : m
    },
    { pattern: /<p>&gt;\s*(.*?)<\/p>/g, replacement: '<blockquote><p>$1</p></blockquote>' },
    { pattern: /<p>&gt;&gt;\s*(.*?)<\/p>/g, replacement: '<blockquote><blockquote><p>$1</p></blockquote></blockquote>' }
]

// 处理多行引用
const processMultiLineQuotes = (html: string): string => html.replace(
    /(<p>&gt;\s*(.*?)<\/p>)+/g,
    (match: string) => `<blockquote>${[...match.matchAll(/<p>&gt;\s*(.*?)<\/p>/g)].map(m => `<p>${m[1]}</p>`).join('')}</blockquote>`
)
    ...
    try {
        ...
        quotePatterns.forEach(({ pattern, replacement, handler }) => {
            processed = replacement
                ...
                : handler ? processed.replace(pattern, handler) : processed
        })
        ...
        processed = processed.replace(
            BLOCKQUOTE_REGEX,
            (m, c) => m.includes('class=') ? m : `<blockquote class="styled-quote">${c}</blockquote>`
        )
