...
export function usePostList(defaultItemsPerPage = 5) {
    ...
    // 获取文章列表
    const fetchData = useCallback(async (page: number, forceRefresh = false) => {
        ...
        try {
            ...
            if (response.code === 0) {
                const { posts, totalPages: pages, currentPage: responsePage } = response.data
                setPostList(posts)
    ...
    // 防抖处理
    const debouncedFetch = useCallback((page: number, forceRefresh = false) => {
    ...
    // 刷新数据（强制获取最新）
    const refreshData = useCallback(() => debouncedFetch(currentPage, true), [currentPage, debouncedFetch])
